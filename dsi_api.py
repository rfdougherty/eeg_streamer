...
class Headset:
    def __init__(self, arg=None):
        ...
        if arg is None or isinstance( arg, str ):
            port = arg.encode('utf-8') if arg!=None else None
            self.ptr = dll.DSI_Headset_New( port )
...
def LoadAPI( dllname = None ):
    ...
    if dllname == None:
        ...
        if platform.architecture()[ 0 ].startswith( '32' ): arch = 'i386'  # even on 32-bit Python for 64-bit Windows, platform.machine() can return 'AMD64'
...
#        print( "DSI Message (level %d): %s" % (lvl, msg) )
#    return 1
#
#@SampleCallback
#def ExampleSampleCallback_Signals( headsetPtr, packetTime, userData ):
#    h = Headset( headsetPtr )
#    strings = [ '%s=%+08.2f' % (ch.GetName(), ch.ReadBuffered() ) for ch in h.Channels() ]
...
#    h = Headset( headsetPtr )
#    fmt = '%s: %3.2f'
#    strings = [ fmt % (src.GetName(), src.GetImpedanceEEG() ) for src in h.Sources() if src.IsReferentialEEG() and not src.IsFactoryReference() ]
